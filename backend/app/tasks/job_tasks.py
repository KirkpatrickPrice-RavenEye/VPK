...
def _secure_cleanup_instance(
    vast_client: VastAIClient, instance_id: int, ssh_key_path: str = None
):
...
"""
        ...
        write_cleanup_cmd = (
            f"cat > /tmp/secure_cleanup.sh << 'EOF'\n{cleanup_script}\nEOF"
        )
        write_result = asyncio.run(
            vast_client.execute_command(instance_id, write_cleanup_cmd, ssh_key_path)
        )
        ...
        if write_result.get("returncode") == 0:
            ...
            cleanup_result = asyncio.run(
                vast_client.execute_command(instance_id, cleanup_cmd, ssh_key_path)
            )
            if cleanup_result.get("returncode") == 0:
                logger.info("Secure cleanup completed successfully")
            else:
                logger.warning(
                    f"Secure cleanup returned non-zero: {cleanup_result.get('stderr', '')}"
                )
            ...
            asyncio.run(
                vast_client.execute_command(
                    instance_id,
                    "shred -vfz -n 3 /tmp/secure_cleanup.sh 2>/dev/null || rm -f /tmp/secure_cleanup.sh",
                    ssh_key_path,
                )
            )
        else:
            logger.warning(
                f"Failed to write cleanup script: {write_result.get('stderr', '')}"
            )
            ...
            """
            asyncio.run(
                vast_client.execute_command(instance_id, basic_cleanup, ssh_key_path)
            )
...
def execute_job(self, job_id: str):
    ...
    try:
        job = (
            db.query(Job)
            .options(joinedload(Job.rule_files))
        ...
        if job.hard_end_time:
            ...
            soft_limit = max(
                60, int(time_remaining - 120)
            )  # 2 minutes before hard limit
            hard_limit = max(
                120, int(time_remaining - 60)
            )  # 1 minute before hard limit

            logger.info(
                f"Setting dynamic timeouts for job {job_id}: soft={soft_limit}s, hard={hard_limit}s"
            )
        ...
        logger.warning(
            f"Soft time limit exceeded for job {job_id}, attempting graceful cleanup..."
        )
        if job:
            try:
    ...
    try:
        ...
        if job.instance_id:
            ...
            try:
                ...
                kill_result = asyncio.run(
                    asyncio.wait_for(
                        vast_client.execute_command(
                            instance_id, "pkill -9 hashcat || true", ssh_key_path
                        ),
                        timeout=3.0,
                    )
                )
            ...
            try:
                ...
                destroy_result = asyncio.run(
                    asyncio.wait_for(
                        vast_client.destroy_instance(instance_id), timeout=5.0
                    )
                )
...
def monitor_job_timeout(job_id: str):
    ...
    try:
        ...
        if job.status not in [
            JobStatus.RUNNING,
            JobStatus.INSTANCE_CREATING,
            JobStatus.QUEUED,
        ]:
            logger.debug(
                f"Job {job_id} is no longer running, stopping timeout monitoring"
            )
            return
        ...
        if job.hard_end_time:
            ...
            if current_time >= job.hard_end_time:
                logger.warning(
                    f"Job {job_id} has exceeded user-defined timeout, triggering cleanup"
                )
                _handle_job_timeout(job, db, is_soft_timeout=True)
                return
            ...
            if time_until_timeout > 30:
                ...
                monitor_job_timeout.apply_async(
                    args=[job_id], countdown=int(time_until_timeout - 5)
                )
        else:
...
def _execute_job_workflow(job: Job, db: Session) -> Dict[str, Any]:
    ...
    try:
        ...
        if job.instance_type:
            ...
            try:
                offers = asyncio.run(
                    vast_client.get_offers(secure_cloud=True, region="global")
                )
                best_offer = next(
                    (offer for offer in offers if offer["id"] == selected_offer_id),
                    None,
                )

                if not best_offer:
                    print(
                        f"Selected offer {selected_offer_id} is no longer available, finding similar offer"
                    )
                    ...
                    try:
                        ...
                        all_offers = asyncio.run(
                            vast_client.get_offers(secure_cloud=True)
                        )
                        original_specs = next(
                            (
                                offer
                    ...
                    if offers:
                        ...
                        if original_specs:
                            ...
                            print(
                                f"Original selection: {original_specs.get('gpu_name')} x{original_gpu_count}, {original_ram}GB RAM"
                            )
                            ...
                            def calculate_similarity_score(offer):
                                ...
                                if (
                                    original_gpu in offer_gpu
                                    or offer_gpu in original_gpu
                                ):
                                ...
                                if offer.get("num_gpus", 1) == original_gpu_count:
                                    ...
                                    abs(offer.get("num_gpus", 1) - original_gpu_count)
                                    == 1
                                ):
                                ...
                                if original_ram > 0:
                                    ram_diff = (
                                        abs(offer_ram - original_ram) / original_ram
                                    )
                            ...
                            offers_with_scores = [
                                (offer, calculate_similarity_score(offer))
                                for offer in offers
                            ]
                            ...
                            print(
                                f"Best match (score {best_score}): {best_offer.get('gpu_name')} x{best_offer.get('num_gpus')}, {best_offer.get('cpu_ram')}GB RAM at ${best_offer.get('dph_total', 0):.3f}/hour"
                            )
                        else:
                            ...
                            budget_offers = [
                                offer
                                for offer in offers
                                if offer.get("dph_total", float("inf"))
                                <= max_cost_per_hour
                            ]
                            ...
                            if budget_offers:
                                ...
                                print(
                                    f"Fallback to cheapest available: {best_offer.get('gpu_name')} at ${best_offer.get('dph_total', 0):.3f}/hour"
                                )
                            else:
                                ...
                                print(
                                    f"Emergency fallback (over budget): {best_offer.get('gpu_name')} at ${best_offer.get('dph_total', 0):.3f}/hour"
                                )
                    else:
                ...
                try:
                    ...
                    best_offer = min(
                        offers, key=lambda x: x.get("dph_total", float("inf"))
                    )
                    print(
                        f"Emergency fallback: Using best available offer {best_offer['id']}"
                    )
            ...
            max_cost_per_hour = settings_service.max_cost_per_hour
            print(
                f"No specific instance selected, getting global secure cloud offers with max cost: {max_cost_per_hour}"
            )
            offers = asyncio.run(
                vast_client.get_offers(
                    secure_cloud=True,
            ...
            if not offers:
                raise Exception(
                    "No suitable secure cloud instances available. This may be due to: 1) No instances in budget, 2) Invalid Vast.ai API key, 3) No secure cloud instances available"
                )
        ...
        if job.hard_end_time:
            ...
            if job.hard_end_time.tzinfo is None:
                ...
                job.hard_end_time = job.hard_end_time.replace(tzinfo=timezone.utc)
            max_duration_hours = (
                job.hard_end_time - current_time
            ).total_seconds() / 3600
        else:
        ...
        print(
            f"Estimated maximum cost: ${estimated_max_cost:.2f} (max allowed: ${max_total_cost})"
        )

        if estimated_max_cost > max_total_cost:
            raise Exception(
                f"Estimated cost ${estimated_max_cost:.2f} exceeds maximum total cost ${max_total_cost}"
            )
        ...
        instance_result = asyncio.run(
            vast_client.create_instance(
        ...
        ready = asyncio.run(
            vast_client.wait_for_instance_ready(instance_id, timeout=600)
        )
        if not ready:
        ...
        try:
            ...
            logger.debug(
                f"Attaching SSH key to instance {instance_id} (key length: {len(public_key)})"
            )
            ...
            if attach_result.returncode == 0:
                ...
                logger.error(
                    f"SSH key attach failed - stderr: {attach_result.stderr}, stdout: {attach_result.stdout}"
                )
        ...
        wait_time = 30
        logger.info(
            f"Waiting {wait_time} seconds for SSH key to be properly configured..."
        )
        import time
        ...
        for attempt in range(2):  # Try 2 times max
            ...
            try:
                test_result = asyncio.run(
                    vast_client.execute_command(instance_id, "echo test", ssh_key_path)
                )
                if test_result.get("returncode") == 0 and "test" in test_result.get(
                    "stdout", ""
                ):
        ...
        if job.hard_end_time:
            ...
            if datetime.now(timezone.utc) > job.hard_end_time:
                ...
                asyncio.run(
                    vast_client.execute_command(
                        instance_id, "pkill -9 hashcat", ssh_key_path
                    )
                )
                job.status = JobStatus.CANCELLED
        ...
        if job.status == JobStatus.CANCELLING:
            print(
                f"Job {job.id} was cancelled during execution, skipping result retrieval (fast retrieval already completed)"
            )
            ...
            try:
                ...
                if instance_info and instance_info.get("actual_status") in [
                    "running",
                    "stopped",
                ]:
                    ...
                    print(
                        f"Instance {instance_id} no longer exists or is not accessible, skipping result retrieval"
                    )
            except Exception as e:
                print(
                    f"Failed to check instance {instance_id} status: {e}, skipping result retrieval"
                )

        # Fix 4: Commit any pot_file_path/log_file_path updates written by
        # _retrieve_results() BEFORE we change the job status to COMPLETED.
        # The SSE generator polls the DB every 2 s; if we write COMPLETED in
        # the same commit as the status change, the frontend may receive the
        # job_finished event while the DB still has no pot_file_path, causing
        ...
        if job.pot_file_path:
            if os.path.isfile(job.pot_file_path):
                pot_size = os.path.getsize(job.pot_file_path)
                print(
                    f"DEBUG: Pot file verified before completion: {job.pot_file_path} ({pot_size} bytes)"
                )
            else:
                print(
                    f"WARNING: Pot file missing or not a regular file at completion: {job.pot_file_path}"
                )
        else:
            print(
                "DEBUG: No pot_file_path set at completion (no passwords cracked or retrieval skipped)"
            )
        ...
        if job.time_started and job.time_finished:
            ...
            if job.time_started.tzinfo is None and job.time_finished.tzinfo is not None:
                job.time_started = job.time_started.replace(
                    tzinfo=job.time_finished.tzinfo
                )
            elif (
                job.time_started.tzinfo is not None and job.time_finished.tzinfo is None
            ):
                job.time_finished = job.time_finished.replace(
                    tzinfo=job.time_started.tzinfo
                )

            duration_hours = (
                job.time_finished - job.time_started
            ).total_seconds() / 3600
        ...
        return {
            ...
            "duration": str(job.time_finished - job.time_started)
            if job.time_started
            else None,
        }
        ...
        if instance_id:
            ...
            if job.status in [JobStatus.CANCELLING, JobStatus.CANCELLED]:
                print(
                    f"Job {job.id} was cancelled, instance {instance_id} likely already destroyed by stop task"
                )
                ...
                try:
                    ...
                    if (
                        instance_info
                        and instance_info.get("actual_status") != "stopped"
                    ):
                    ...
                    print(
                        f"Instance {instance_id} check/destruction failed (likely already destroyed): {e}"
                    )
            else:
                ...
                try:
                    ...
                    print(
                        f"Secure cleanup failed for instance {instance_id}: {secure_cleanup_error}"
                    )
...
def _setup_instance(
    ...
    try:
        ...
        mkdir_result = asyncio.run(
            vast_client.execute_command(
                instance_id, "mkdir -p /workspace", ssh_key_path
            )
        )
        ...
        result = asyncio.run(
            vast_client.execute_command(instance_id, "ls -la /workspace", ssh_key_path)
        )
    ...
    try:
        ...
        setup_result = asyncio.run(
            vast_client.execute_command(instance_id, setup_tmpfs_cmd, ssh_key_path)
        )
        ...
        if setup_result.get("returncode") != 0:
            ...
            print(
                f"CRITICAL: tmpfs setup failed - cannot proceed without secure memory storage"
            )
            print(f"tmpfs error: {error_msg}")
            raise Exception(
                "Critical security requirement failed: tmpfs mount unsuccessful. Cannot store sensitive hash files on disk."
            )
        ...
        if result.returncode != 0:
            ...
            raise Exception(
                f"Failed to stream hash data: {filtered_stderr or 'Unknown error'}"
            )
        ...
        verify_result = asyncio.run(
            vast_client.execute_command(instance_id, verify_cmd, ssh_key_path)
        )

        if verify_result.get("returncode") != 0:
            logger.error(
                f"Hash file verification failed: {verify_result.get('stderr', '')}"
            )
            raise Exception(
                "Hash file was not properly transferred via SSH streaming - verification failed"
            )
        else:
            logger.info(
                f"Successfully streamed and verified hash file to instance (secure: {'tmpfs' if setup_result.get('returncode') == 0 else 'workspace'})"
            )
    ...
    if job.word_list:
        ...
        try:
            object_key = job.word_list
            presigned_url = S3Client().get_download_url(object_key)
            logger.info(f"Generated presigned URL for {object_key}")

            wordlist_filename = object_key.split("/")[-1]
            local_path = f"/workspace/{wordlist_filename}"
            ...
            file_exists = file_size_str.isdigit() and int(file_size_str) > 0

            if not file_exists:
                stderr = download_result.get("stderr", "Unknown error")
                raise Exception(
                    f"Wordlist {object_key} was not properly downloaded (file missing or empty). "
                    f"Download stderr: {stderr[:300]}"
                )

            # Check file size locally after download
            size_cmd = f"stat -c %s {local_path}"
            size_result = asyncio.run(
                vast_client.execute_command(instance_id, size_cmd, ssh_key_path)
            )

            if size_result.get("returncode") == 0:
                try:
                    file_size_bytes = int(size_result.get("stdout", "0").strip())
                    file_size_gb = file_size_bytes / (1024 * 1024 * 1024)
                    if file_size_gb >= 1:
                        job.status_message = (
                            f"Downloaded large wordlist ({file_size_gb:.1f} GB)"
                        )
                    else:
                ...
                print("Could not determine file size after download.")

            # Final verification
            verify_cmd = f"ls -la {local_path}"
            verify_result = asyncio.run(
                vast_client.execute_command(instance_id, verify_cmd, ssh_key_path)
            )
            if verify_result.get("returncode") != 0:
                raise Exception("Wordlist verification failed after download.")
            else:
                print("Wordlist successfully downloaded and verified.")
    ...
    if job.word_list:
        try:
            ...
            wordlist_filename = job.word_list.split("/")[
                -1
            ...
            if compression_format:
                ...
                extract_result = asyncio.run(
                    vast_client.execute_command(instance_id, extract_cmd, ssh_key_path)
                )
                ...
                if extract_result.get("returncode") != 0:
                    ...
                    raise Exception(
                        f"Failed to extract compressed wordlist: {error_msg[:200]}"
                    )

                # Verify extraction worked
                verify_extract = asyncio.run(
                    vast_client.execute_command(
                        instance_id, f"ls -la '{output_path}'", ssh_key_path
                    )
                )
                if verify_extract.get("returncode") != 0:
                    raise Exception(
                        "Wordlist extraction failed - extracted file not found"
                    )

                print(
                    f"Successfully extracted {compression_format} wordlist to {base_name}"
                )
    ...
    rule_files_list = (
        [rule.rule_file for rule in job.rule_files] if job.rule_files else []
    )
    if rule_files_list:
        ...
        for i, rule_file in enumerate(rule_files_list):
            try:
                ...
                print(
                    f"Downloading rule file {i + 1}/{len(rule_files_list)}: {rule_file} (presigned URL)"
                )

                # Generate presigned URL for the rule file
                presigned_url = S3Client().get_download_url(rule_file)
                ...
                if not file_exists:
                    stderr = download_result.get("stderr", "Unknown error")
                    raise Exception(
                        f"Rule file {rule_file} was not properly downloaded (file missing or empty). "
                        f"Download stderr: {stderr[:300]}"
                    )
                else:
                    print(
                ...
                raise Exception(
                    f"Critical: Cannot proceed without rule file {rule_file} - {str(e)}"
                )
...
def _execute_hashcat(
    ...
    cmd_parts = hashcat_service.build_command(
        job_for_hashcat, wordlist_path, rules_paths, skip_validation=True
    )
    ...
    if job.hard_end_time:
        ...
        if job.hard_end_time.tzinfo is None:
            job.hard_end_time = job.hard_end_time.replace(tzinfo=timezone.utc)
        remaining_time = (
            job.hard_end_time - datetime.now(timezone.utc)
        ).total_seconds()
        timeout_seconds = min(timeout_seconds, max(60, remaining_time))
        print(
            f"Setting hashcat timeout to {timeout_seconds} seconds based on hard_end_time"
        )
    else:
        print(
            f"No hard_end_time set, using default timeout of {timeout_seconds} seconds"
        )

    # Verify files exist before running hashcat
    try:
        print("DEBUG: Verifying required files exist...")
        file_check = asyncio.run(
            vast_client.execute_command(instance_id, "ls -la /workspace/", ssh_key_path)
        )
        print(f"DEBUG: Workspace contents before hashcat: {file_check}")

        # Check specific files
        hash_check = asyncio.run(
            vast_client.execute_command(
                instance_id, "ls -la /workspace/hashes.txt", ssh_key_path
            )
        )
        print(f"DEBUG: Hash file check: {hash_check}")

        if job.word_list and wordlist_path:
            wordlist_check = asyncio.run(
                vast_client.execute_command(
                    instance_id, f"ls -la {wordlist_path}", ssh_key_path
                )
            )
...
"""
    ...
    write_script_cmd = (
        f"cat > /workspace/run_hashcat.sh << 'EOF'\n{wrapper_script}\nEOF"
    )
    script_result = asyncio.run(
        vast_client.execute_command(instance_id, write_script_cmd, ssh_key_path)
    )
    ...
    start_result = asyncio.run(
        vast_client.execute_command(instance_id, start_cmd, ssh_key_path)
    )
    print(f"DEBUG: Background start result: {start_result}")

    if start_result.get("returncode") != 0:
        raise Exception(
            f"Failed to start hashcat in background: {start_result.get('stderr', 'Unknown error')}"
        )
    ...
    while monitoring_loops < max_monitoring_loops:
        ...
        try:
            ...
            if job.hard_end_time:
                ...
                if datetime.now(timezone.utc) > job.hard_end_time:
                    print(
                        f"HARD TIME LIMIT EXCEEDED for job {job.id} - Force stopping hashcat"
                    )
                    # Kill hashcat process
                    kill_result = asyncio.run(
                        vast_client.execute_command(
            ...
            pid_check = asyncio.run(
                vast_client.execute_command(
                    instance_id,
                    "cat /workspace/hashcat.pid 2>/dev/null || echo 'NO_PID'",
                    ssh_key_path,
                )
            )
            ...
            process_result = asyncio.run(
                vast_client.execute_command(
                    instance_id, check_process_cmd, ssh_key_path
                )
            )
            ...
            marker_check = asyncio.run(
                vast_client.execute_command(
                    instance_id,
                    "test -f /workspace/hashcat.running && echo 'MARKER_EXISTS' || echo 'MARKER_GONE'",
                    ssh_key_path,
                )
            )
            ...
            tail_result = asyncio.run(
                vast_client.execute_command(instance_id, tail_cmd, ssh_key_path)
            )
            ...
            if tail_result.get("returncode") == 0:
                ...
                print(
                    f"Failed to read hashcat output (attempt {consecutive_failures}/{max_consecutive_failures})"
                )
                print(
                    f"DEBUG: Tail error: {tail_result.get('stderr', 'Unknown error')}"
                )
            ...
            if not is_running:
                ...
                final_result = asyncio.run(
                    vast_client.execute_command(instance_id, final_cmd, ssh_key_path)
                )
                ...
                workspace_files = asyncio.run(
                    vast_client.execute_command(
                        instance_id, "ls -la /workspace/", ssh_key_path
                    )
                )
            ...
            if consecutive_failures >= max_consecutive_failures:
                print(
                    f"Too many consecutive failures ({consecutive_failures}), stopping monitoring"
                )
                job.error_message = (
                    "Lost connection to hashcat process during monitoring"
                )
                job.status = JobStatus.FAILED
                db.commit()
                break
            ...
            consecutive_failures += 1
            print(
                f"Error during monitoring (attempt {consecutive_failures}/{max_consecutive_failures}): {e}"
            )
...
def _parse_hashcat_progress_realtime(output: str, job: Job, db: Session):
    ...
    try:
        ...
        if status_lines:
            ...
            if (
                ...
                if status_code == 5:  # Exhausted
                    ...
                    job.status_message = (
                        f"Completed: {progress_pct}% - All hashes cracked{speed_msg}"
                    )
                    job.progress = 100
                    ...
                    job.status_message = (
                        f"Auto-tuning GPU performance: {progress_pct}%{speed_msg}"
                    )
                else:
                    job.status_message = (
                        f"Processing: {progress_pct}% complete{speed_msg}{eta_msg}"
                    )
        ...
        if (
            "Initializing device kernels" in output
            or "Initializing backend runtime" in output
        ):
        ...
        if "STATUS" in stdout:
            ...
            if status_lines:
                ...
                if (
                    ...
                    progress_pct = min(
                        95, int((progress_current / progress_total) * 100)
                    )
...
def _retrieve_results(
    ...
    try:
        list_result = asyncio.run(
            vast_client.execute_command(
                instance_id,
                "find /workspace -type f -name '*.pot' -o -name '*.txt' -o -name '*.log' | head -20",
                ssh_key_path,
            )
        )
    ...
    for pot_location in pot_locations:
        try:
            ...
            check_result = asyncio.run(
                vast_client.execute_command(
                    instance_id,
                    f"test -f {pot_location} && echo 'EXISTS' || echo 'NOT_FOUND'",
                    ssh_key_path,
                )
            )
            ...
            if check_result.get("stderr"):
                logger.warning(
                    f"SSH stderr while checking file: {check_result['stderr']}"
                )
            ...
            if result.returncode == 0 and os.path.isfile(tmp_scp_path):
                ...
                if file_size == 0:
                    print(
                        f"Successfully retrieved empty pot file from {pot_location} (no passwords cracked)"
                    )
                else:
                    print(
    ...
    try:
        ...
        for log_location in log_locations:
            try:
                ...
                result = subprocess.run(copy_cmd, capture_output=True, text=True)
                print(
                    f"DEBUG: Log copy from {log_location} - return code: {result.returncode}"
                )
                ...
                if result.returncode == 0 and os.path.isfile(log_file_path):
                    ...
                    try:
                        with open(log_file_path, "r") as f:
                            log_content = f.read()
                            print(
                                f"DEBUG: Log file contents ({len(log_content)} chars):"
                            )
                            print(f"--- BEGIN LOG ---")
                            print(log_content[:2000])  # Show first 2000 chars
                            if len(log_content) > 2000:
                                print(
                                    f"... (truncated, total {len(log_content)} chars)"
                                )
                            print(f"--- END LOG ---")
                    except Exception as e:
...
def stop_job(job_id: str):
    ...
    try:
        ...
        if job.instance_id and job.status in [
            JobStatus.RUNNING,
            JobStatus.INSTANCE_CREATING,
        ]:
            vast_client = VastAIClient()
            ...
            try:
                ...
                if not os.path.exists(ssh_key_path):
                    print(
                        f"Generating new SSH key for stop operation on instance {instance_id}"
                    )
                    os.makedirs(instance_ssh_dir, exist_ok=True)
                    ssh_pub_key_path = f"{instance_ssh_dir}/id_rsa.pub"
                    ...
                    if keygen_result.returncode == 0:
                        ...
                        if attach_result.returncode != 0:
                            print(
                                f"Failed to attach stop SSH key: {attach_result.stderr}"
                            )
                        ...
                        print(
                            f"Failed to generate stop SSH key: {keygen_result.stderr}"
                        )
                        ssh_key_path = None
                else:
            ...
            try:
                asyncio.run(
                    asyncio.wait_for(
                        vast_client.destroy_instance(instance_id), timeout=10.0
                    )
                )
            except Exception as e:
...
def _retrieve_results_fast(
        ...
        try:
            ...
            check_result = asyncio.run(
                asyncio.wait_for(
                    vast_client.execute_command(
                        instance_id,
                        f"test -f {log_location} && echo 'EXISTS' || echo 'NOT_FOUND'",
                        ssh_key_path,
                    ),
                    timeout=3.0,
                )
            ...
            if "EXISTS" in check_result.get("stdout", ""):
                ...
                result = subprocess.run(
                    scp_cmd, capture_output=True, text=True, timeout=10.0
                )
                if result.returncode == 0 and os.path.isfile(log_file_path):
                    job.log_file_path = log_file_path
                    file_size = os.path.getsize(log_file_path)
                    print(
                        f"Successfully retrieved log file from {log_location} ({file_size} bytes)"
                    )
                    log_retrieved = True
                    break
                else:
    ...
    if pot_retrieved and log_retrieved:
        ...
        print(
            f"Failed to retrieve any result files for job {job.id} within 1-minute timeout"
        )
