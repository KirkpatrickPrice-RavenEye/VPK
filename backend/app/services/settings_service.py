...
class SettingsService:
    ...
    def _get_settings(self) -> ApplicationSetting:
        ...
        if self._settings_cache is None:
            settings = (
                ...
                .filter(ApplicationSetting.id == 1)
                .first()
            )
            if not settings:
    ...
    def teams_webhook_url(self) -> Optional[str]:
        return self._get_settings().teams_webhook_url
    ...
    def get_teams_webhook_url(self) -> Optional[str]:
        """Get Microsoft Teams webhook URL from database or fallback to env"""
        db_value = self.teams_webhook_url
        if db_value:
            return db_value
