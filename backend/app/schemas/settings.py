...
class SettingsUpdate(BaseModel):
    ...
    max_cost_per_hour: Optional[float] = Field(
        None, ge=0.01, le=1000.0, description="Maximum cost per hour in USD"
    )
    max_total_cost: Optional[float] = Field(
        None, ge=1.0, le=100000.0, description="Maximum total cost in USD"
    )

    # File size limits (in MB)
    max_upload_size_mb: Optional[int] = Field(
        None, ge=1, le=10000, description="Maximum upload size in MB"
    )
    max_hash_file_size_mb: Optional[int] = Field(
        None, ge=1, le=1000, description="Maximum hash file size in MB"
    )

    # Data retention
    data_retention_days: Optional[int] = Field(
        None, ge=1, le=365, description="Data retention period in days"
    )

    # AWS S3 settings
    s3_bucket_name: Optional[str] = Field(
        None, max_length=255, description="S3 bucket name"
    )
    s3_region: Optional[str] = Field(None, max_length=50, description="AWS region")

    # Vast.ai settings
    vast_cloud_connection_id: Optional[str] = Field(
        None, max_length=255, description="Vast.ai cloud connection ID"
    )

    # Sensitive fields (will be encrypted)
    aws_access_key_id: Optional[str] = Field(
        None, max_length=128, description="AWS access key ID"
    )
    aws_secret_access_key: Optional[str] = Field(
        None, max_length=128, description="AWS secret access key"
    )
    vast_api_key: Optional[str] = Field(
        None, max_length=128, description="Vast.ai API key"
    )
    teams_webhook_url: Optional[str] = Field(
        None, max_length=1024, description="Microsoft Teams incoming webhook URL"
    )
...
class SettingsResponse(BaseModel):
    ...
    teams_webhook_configured: bool = Field(
        description="Whether Microsoft Teams webhook URL is configured"
    )
...
class ConnectionTestResponse(BaseModel):
    ...
    class Config:
        schema_extra = {
            "example": {"status": "success", "message": "AWS S3 connection successful"}
        }
