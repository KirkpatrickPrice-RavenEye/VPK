...
async def generate_job_events(job_id: str, user_id: str) -> AsyncGenerator[str, None]:
    ...
    last_cracked_count = 0  # Track highest cracked count (monotonically increasing)
    last_pot_preview = []  # Cache last valid preview lines
    ...
    while True:
        try:
            ...
            if (
                not initial_log_sent
                and job.log_file_path
                and os.path.exists(job.log_file_path)
            ):
                try:
            ...
            if job.pot_file_path and os.path.exists(job.pot_file_path):
                try:
                    ...
                    if current_hash != last_pot_hash:
                        ...
                        # Guard: cracked count should only increase during a job.
                        # If we read fewer lines than before, the pot file was likely
                        # caught mid-write (truncated). Skip this update and keep the
                        # previous hash so we re-check next cycle.
                        if len(lines) < last_cracked_count:
                            print(
                                f"Pot file race detected: read {len(lines)} lines but "
                                f"last known count was {last_cracked_count}, skipping update"
                            )
                        else:
                            last_cracked_count = len(lines)
                            last_pot_preview = preview_lines
...
async def stream_all_jobs_events(current_user: User = Depends(get_current_active_user)):
        ...
        while True:
            try:
                ...
                for job in jobs:
                    ...
                    if (
                        job_id not in last_jobs_state
                        or last_jobs_state[job_id] != current_state
                    ):
                        event_data = {
                            "job_id": job_id,
                            "name": job.name,
